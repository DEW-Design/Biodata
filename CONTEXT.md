...
- **Sept 25 2026: GitHub Pages build fixed (two static-export errors), reported from the failing Actions run.** The static export (`PAGES_BASE_PATH` set, `output: "export"`) needs every dynamic route to list its ids and every `useSearchParams` caller to sit in a Suspense boundary. (1) `/pages/dla/[id]/edit` was still a client-only page with no `generateStaticParams()`; it now follows the DSA edit route: a server wrapper (`page.tsx`) lists `staticDlaIds()` and the client page moved to `edit-dla-page.tsx`. Checked every `[id]` route (DSA and DLA detail and edit): all four now have it. (2) Once that passed, the build failed on `/pages/observations/option-5` ("useSearchParams() should be wrapped in a suspense boundary"): the Explore layout switcher reads the role from the URL and sat outside the page's Suspense; `ExploreLayoutSwitcher` now has its own boundary (every Explore option page was exposed to this, only the first to prerender failed). **Verified with the real Pages build** (`PAGES_BASE_PATH=/Biodata npm run build`): all 533 routes prerender. Lesson for the checklist: run that build before pushing a new route or a client hook that reads the URL; `tsc`, `eslint` and the contract check do not catch either error.
- **Sept 25 2026: a "Pages build" check now runs on every pull request into main (`.github/workflows/pages-build-check.yml`).** The Pages deploy only runs after a push to main and the design system audit does not build the site, so two static-export errors (a dynamic route with no `generateStaticParams`, a `useSearchParams` caller outside Suspense) reached main and failed only after the merge. The new workflow runs the same build as the deploy (Node 20, `next build` with `PAGES_BASE_PATH=/<repo>`) without deploying, cancels superseded runs on the same branch, and its job is named "Pages build" so it can be made a required check. **Making it required needs a branch rule on main (repo admin); the workflow alone reports a red check but does not block a merge.**
- **Sept 25 2026: `TextArea` typography now matches `Input`, per page feedback ("the typography in text area looks dubious").** Measured live (computed styles, not read from classes): the `Input` renders at 14px Barlow with `px-3 py-2`, while `TextArea` rendered 16px with `px-3.5 py-3` (and `size="sm"` was `p-3`), so a textarea beside an input looked a size larger. Cause: `Input`'s `md` and `lg` sizes ask for the undefined `text-md` and silently fall back to 14px, while `TextArea` used the real `text-base` (16px). Fixed in `components/base/textarea/textarea.tsx`: both sizes are now `px-3 py-2 text-sm` (14px, 8px and 12px padding, same as Input's rendered result); font family was already Barlow in both (it comes from the wrapper). Every `TextArea` consumer (DSA, DLA, Add Project, project-detail editors) picks it up. Sibling grep: `TextArea` was the only field on the real `text-base`; `Input`, `InputNumber`, `InputDate`, `InputTags`, `InputGroup`, `PinInput`, `NativeSelect`, `Select`, `MultiSelect`, `Checkbox` and `RadioButton` all use the dead `text-md` (a known, ratcheted class). **Open, not changed:** whether fields should be 14px or 16px is a designer decision; making `text-md` real would change every field at once. `tsc`, `eslint` and `npm run check:contracts` clean.
- **Sept 25 2026: "Upload dataset" moves from the Projects list to the project itself, per page feedback ("upload dataset should sit on a project level") and the designer's choice (a visible button beside a "..." menu).** A dataset always belongs to one project, so the Projects list's Actions group (column 2) no longer has it; it keeps Export CSV and Create report. On `/pages/project-detail` the gradient card now has, at its top right, a secondary "Upload dataset" button (the recurring task, one click) and a "..." menu (`Dropdown`) holding Export CSV (this project's row as `BD-5039.csv`). New page-shared `app/pages/_shared/project-card-actions.tsx` (`ProjectCardActions`; not under `components/**`, so no override). A guest still sees both and gets the sign-up invite on click (Upload dataset, Export data); registered users get the same "not built yet" toast as before, since upload itself is still not wired. Note: the Home card precedent says a banner carries no action buttons, but that banner had its actions in the header; here the card is the project's own identity and the designer asked for actions on it, so the two do not conflict. `ProjectActions` lost its upload state and modal. Verified live (admin and guest, 1708px): no Upload dataset on the list, Export CSV still there; the card shows the button and menu; admin upload shows the toast and export downloads `BD-5039.csv`; guest upload and export open their invites; screenshot checked; zero console errors; `tsc`, `eslint` and `npm run check:contracts` clean. **Open:** the project-detail sibling routes (options 2 and 3) have no upload action yet; the "..." menu only has Export CSV, so Edit project or Add to project could go there later; not committed.
- **Sept 25 2026: the Explore results panel's tab strip fades out on the right while there is more to scroll, per page feedback on option 3 ("add a gradient wash from the right, as a way to invite scroll + show that there's content there").** The Species / Projects / Events / Occurrences / Observations / Artefacts row scrolls sideways in the narrow card and the cut-off "Occurrences" read as a clipped label. New page-shared `app/pages/_shared/map-search/scroll-fade.tsx` (`ScrollFade`): the strip gets a 64px right-edge fade drawn as a `mask-image` (not an overlay, so it works on any background), shown only while `scrollLeft + clientWidth < scrollWidth` and removed once scrolled to the end or when everything fits (tracked on scroll and with a `ResizeObserver`); the scrollbar stays hidden. It wraps `panelTabsEl`, which every Explore layout with results shares, so options 2, 4 and 5 get it too (the strip only overflows in the narrow cards). Verified live on option 3: mask present at scrollLeft 0 (398px shown of 667px), `none` at the end, screenshot shows "Occurrences" fading out, zero console errors; `tsc`, `eslint` and `npm run check:contracts` clean. Not committed (on the `project-upload-action` branch with the Upload dataset change).
- **Sept 25 2026: the Explore search setup (method tabs, area chips, keyword) is cleaned up, per designer feedback on option 3 ("a cluster mess of icons with no consideration to human interface principles").** Problems in the screenshot: the four methods were bare glyphs (a pen, a pin, an unreadable map symbol, an upload cloud) that do not say which method is which (recognition over recall), the chosen method was then named again in a heading, the area chip carried a pin icon that added nothing, and the keyword field had only a placeholder for a label (it vanishes on typing). Changes in `observations-search.tsx`, shared by options 2 to 5 (option 1 is untouched and keeps its icons and labels): **named tabs, no icons** (Draw, Coordinates, Location, Shapefile; the chosen tab already names the method, so the heading under it is gone); **one hint line under Circle and Polygon that is always there** and changes with the tool ("Pick a shape, then draw it on the map." / the drawing instructions), replacing text that only appeared while drawing; **area chips are text and a remove button** (no leading pin icon, 14px text); **a real "Species or keyword" label** on the two card layouts (options 3 and 4; the option 5 bar and option 2's column keep the placeholder-only field, where a label does not fit). Option 2's 286px column cannot fit four named tabs in a row (measured 332px needed against 253px), so it uses the same tabs stacked vertically at full width, the pattern Home's switcher already uses. Verified live: option 3 tabs fit (366px), option 2 stack at 253px, option 1 still has its 4 icons, screenshots checked, zero console errors; `tsc`, `eslint` and `npm run check:contracts` clean. **Open:** the Circle and Polygon buttons keep their shape icons, which do describe the tool; the vertical tabs in option 2 make that column taller. Not committed (on `project-upload-action` with the two earlier changes).
- **Sept 25 2026: Explore options 3, 4 and 5 have ONE search field, with the areas as chips inside it, per designer feedback ("why two search fields?") and the agreed Mobbin pattern list (steps 1 and 2).** The card had "Species or keyword" (what to look for) and, in the results, "Search species" (narrows the list that field produced): two boxes, one mental model. Patterns taken (ideas only, CONTRACTS 2.6): Turo, komoot, GetYourGuide and Zillow keep one search for the whole screen and give the results filters and a count, no second box; Kiwi.com shows the chosen area as a token inside the search field ("San Diego +250 km"). Now: a magnifier, the areas as removable chips, then the text, in one field (`SearchWithAreas`, `observations-search.tsx`); Backspace in an empty field takes the last area off; "Clear all" appears with more than one area. Typing narrows the results live, as the keyword already did. The results controls row lost its search box (`resultsBody` `showSearch: false` for options 3 to 5; Filters and the List/Table toggle stay), and `SpeciesResultsView` gained an additive `hideSearch` prop (default off) so the sheet's species table drops its own box too. Option 5's narrow bar shows the first area and "+N", and its Areas popover still manages them all. **Composed, not a real component (CONTRACTS 1.2):** no DEW component puts non-typed removable chips beside typed text (`InputTags` turns typed text into tags); the field uses `Input`'s ring, radius and shadow tokens, the same "composed from real tokens, logged" treatment as this file's other bespoke controls, and is a candidate for a real component if the designer wants one. **Not changed:** option 2 (its keyword sits in column 2 and its results still have their own box) and option 1. **Step 3 of the proposal (Circle and Polygon as map tools, an Add area menu) is not started.** Verified live on options 3, 4 and 5: exactly one search textbox each, the area chip and keyword in one field, Backspace removes the area, screenshots checked, zero console errors; `tsc`, `eslint` and `npm run check:contracts` clean. Not committed (on `project-upload-action` with the earlier changes).
- **Sept 25 2026: component behaviour patterns are now a contract (CONTRACTS.md 1.9), and the Escape-clears-the-selection bug is fixed at the component, per designer feedback ("follow the correct behaviour pattern... this applies to all design system components... if it isn't there, add it").** Read as: the standing gap logged six times above ("pressing Escape in a `MultiSelect` clears its selection", never fixed) is a behaviour-pattern violation, and there was no clause that said components must behave as their standard pattern. **Cause:** react-aria's `ListBox` defaults `escapeKeyBehavior` to `"clearSelection"`, so Escape inside an open `MultiSelect` wiped everything chosen. **Fix, once, in the components:** `escapeKeyBehavior="none"` on the multiple-selection `ListBox` in `MultiSelect` and `TagSelect` (the only two in `components/**`), so every consumer (Explore's park and filter pickers, Add Project, the DSA and DLA forms and more) gets it with no call-site change. **Contract 1.9:** a component behaves as its WAI-ARIA pattern; Escape closes and never changes a committed value; selection applies live and closing keeps it; the keyboard is complete (Tab, Arrows, Enter, Space, focus back to the trigger); every state is real; a behaviour change is tested by keyboard on every screen that uses the component. Origin lines name this bug and `Tab`'s `badge={0}` "0". **Enforcement:** `AUTO §1.9a` (`scripts/check-contracts.mjs`) fails a multiple-selection `ListBox` in `components/**` without `escapeKeyBehavior`; verified it fires when the prop is removed and passes when restored. Adding a clause is not a loosening (9.4), but the change to the contract file and script is itself flagged in the next audit. **Not covered (no automatic check):** the other items in 1.9 are `REVIEW`. **Left as is on purpose:** Escape clearing a table or tree row selection is react-aria's standard grid and tree behaviour, not a popover, so it stays. Verified live on Explore option 3 (Location, 17 parks): two parks selected, Escape closes the popover and both stay selected (2 area chips), zero console errors; `tsc`, `eslint` and `npm run check:contracts` clean. Not committed.
- **Sept 25 2026: Explore keyword-only search, and the map fits areas into the part of the map you can see.** (1) In options 2 to 5 a word with no area is now a valid search: it searches all of South Australia and shows an "All of South Australia" chip (removing it clears the word); an explicit area replaces it, and both together narrow. Option 1 keeps its Search button. (2) Per designer feedback off a screenshot (option 3 in Table mode, the drawn circle hidden behind the 960px card; Airbnb puts results in the empty part of the map): `FlyToBoundaries` in `sa-map.tsx` now re-fits whenever the covered area changes, and caps each padding at 78% of the map. Option 3 passes the real card width in Table mode (it passed 48px before), and option 4 passes the sheet's height for its current snap. Verified live on option 3 (Belair National Park, Table mode): the circle and its pin sit in the open strip on the right, zero console errors; `tsc`, `eslint`, `check:contracts` clean. **Known:** at 1600px the visible strip in Table mode is only about 300px, so a large area fits small; the card could fold to a list when the user wants the map. Not committed.
- **Sept 25 2026: Explore option 3 shows a picked record as a short summary card first, and the full record only on request, per designer feedback ("the side panel takes so much space, hides all the stuff ... just enough information then lead the user to the actual information").** Clicking a result row, a list card or a map dot used to open `RecordDetailSidebar` (a 900px modal with the full accordion, blurring the map and the results). In option 3 it now opens `RecordPeekCard` (`app/pages/_shared/map-search/record-peek-card.tsx`, page-shared): a 320px non-modal card top-right of the map with the type, name (scientific name for species), provenance (organisation and project), three facts (an event: ID, start date, status or region; an occurrence: date, region, count; an observation: date, region, observer), a Restricted note for Level 2 species, and one primary "View full record" button that opens the existing full sidebar. The map and results stay usable behind it; the selected dot is drawn larger and darker; picking another record swaps the card; the X or Escape closes it (closing the full record returns to the card). Options 1, 2, 4 and 5 are unchanged so the flows can be compared (CONTRACTS 4.4: rolling it to the others is a decision for the designer). Composed from real `Button` and `Badge`, no new `components/**` file. Verified live on option 3 (Belair National Park, List mode): card opens on a list click with the right facts, the full record opens only from the button and the card hides, zero console errors; `tsc` and `check:contracts` clean. **Not covered:** clicking a table row and a map dot use the same path but were not clicked separately; Artefacts still open their own viewer. Not committed.
- **Sept 25 2026: "View full record" on the Explore option 3 summary card goes to the project page, per designer feedback ("clicking on view full record should take me directly to the project detail").** The button is now a real link to `/pages/project-detail` (with `?userRole=` carried, and `&select=` pre-selecting the record in the project's tree where the two datasets share a code: today only Site `SU00501` and Visit `VU00501`). The route the designer wrote, `/projects/[id]/project-detail`, does not exist; the real page is `/pages/project-detail` (CONTRACTS 0.2), and only Adelaide Hills Bushland Survey has one in this preview. **For a record in any other project the button still opens the full record sidebar** rather than a dead or wrong link (the same honesty rule as the sidebar's disabled "Go to project"); this is a decision worth confirming, and it goes away once those projects have pages. New shared `projectDetailPath(record)` in `record-detail.tsx` is the one source for both the card's button and the sidebar's "Go to project", so they cannot disagree. Verified live on option 3: an Adelaide Hills occurrence gives a link to `/pages/project-detail?userRole=registered-user` and lands there; a Kangaroo Island one shows the button and opens the sidebar; zero console errors; `tsc`, `eslint`, `check:contracts` clean. Not committed.
- **Sept 25 2026: species reference photos in Explore, from the Atlas of Living Australia (ALA), per designer direction ("build it now with an imageUrl field ... and the icon fallback. Use ALA if you can").** A species can be recognised by sight: the option 3 summary card leads with a photo (full width, 144px tall, with the credit under it: creator, licence, and a link to the image's ALA page), and the list cards for Species, Occurrences and Observations show a 40px thumbnail in place of the group icon. **Real images only (CONTRACTS 0.3):** for each of the 21 species in `search-data.ts`, ALA's own search gave the representative image for that exact scientific name, and its image metadata gave the creator and licence. **Kept:** CC BY and CC BY-NC (14 species, copies in `public/pages/species/`, about 900 KB in all, so the prototype does not depend on ALA being up). **Excluded on purpose:** images with no licence recorded (Macropus giganteus, Litoria raniformis, Acacia pycnantha) and CC BY-NC-ND (Emu: a cropped thumbnail is a derivative). **No ALA image under that name** (Macropus rufus, Neophema chrysogaster, Malurus cyaneus). Those species, and every non-species record, fall back to the group icon on a same-size tile, so nothing shifts or breaks. New page-shared files: `map-search/species-images.ts` (the map from scientific name to file, credit and licence; the one place to add or swap a photo), `map-search/species-photo.tsx` (`SpeciesPhoto`: fixed size, alt text is the common name, lazy, icon fallback); `ResultCard` gained an optional `thumb`, and `RecordPeekCard` shows the photo and credit. Verified live on option 3 (Belair National Park): thumbnails on 3 of 6 species cards, the others show their icon tile; the Echidna card opens with its photo and credit; zero console errors; `tsc`, `eslint`, `check:contracts` clean.
  - **For the designer to decide:** (1) **CC BY-NC (9 of the 14) covers non-commercial use only.** Whether a government service counts is a licensing call for DEW, not made here; swapping to DEW's own photo library is a one-file change in `species-images.ts`. (2) **A data problem the photos exposed:** the record "Western Grey Kangaroo" carries the scientific name Macropus giganteus, which is the Eastern Grey Kangaroo (the Western Grey is Macropus fuliginosus). ALA's image for that name is unlicensed so it is not shown, but the mismatch is in `search-data.ts` and was left alone (it changes a record). (3) Some ALA "representative" images are poor for identification (the Fairy Tern is a small bird in a large sky); ALA has others per species, and a curated choice would be better than the auto-picked one. (4) Not yet in the species table, the header search results or the record sidebar. Not committed.
- **Sept 25 2026: the summary card's photo corners follow the outer radius, per designer feedback off a screenshot ("inner corner radius = outer - padding").** The photo sat inside the card's 16px padding with an 8px radius, but the card's outer radius is 12px, which is smaller than the padding, so no positive inner radius is concentric (12 - 16 is negative) and the rounded photo looked wrong against the card corner. The photo is now full-bleed at the top of the card (`overflow-hidden` on the card, `rounded-none` on the image), so its top corners are the card's own 12px radius by construction; the credit line and the rest of the content keep the 16px padding in their own block. Measured live: card 12px, image 0px, clipped by the card. Sibling check: the 40px list thumbnails keep their 8px radius (they sit in a row whose hover fill is the only surface, not inside a padded card). `tsc`, `eslint`, `check:contracts` clean. Not committed.
- **Sept 25 2026: the summary card's close button is always the card's top-right corner, per designer feedback off a screenshot ("the x should always be top right").** With a photo it had ended up beside the eyebrow text below the image. It is now positioned on the card itself (8px from the top and right), over the photo on a solid white chip when there is one (a bare grey X vanished on the photo, and `bg-primary/90` is a dead class: the opacity modifier does not work on these utilities) and over the plain header when there is not (the title block reserves room so text never runs under it). 32px visible, 44px hit area. Measured live: 9px from the top and right edges with and without a photo; clicking it closes the card. Escape still closes. `tsc`, `eslint`, `check:contracts` clean. Not committed.
- **Sept 25 2026: Explore option 3 treats search areas as layers, and adding one is one menu, per designer direction ("we'll have to treat multiple points as layers ... the experience needs to be tightened up for adding multiple points / shapefile / choosing an area").** Built on option 3 only (`float-grow`); options 1, 2, 4 and 5 keep their controls.
  - **Areas list (column 2, `map-search/area-layers.tsx`, page-shared):** one row per area you added (a drawn circle or polygon, an entered point, each selected national park, each shapefile). A row has a show/hide toggle, a name ("Circle 1", "Point 1", the park's name, the file name), what it is ("25 km radius", "Shapefile · 2 points") with its record count, and a "..." menu (Rename, Change radius, Remove). Pointing at a row draws that shape heavier on the map; clicking the name zooms to it. "Clear all" appears with more than one. **Unlike a design tool, order means nothing:** the search is the union of the visible areas, so rows are not stacked or reordered. A hidden area is neither drawn nor searched; if every area is hidden the card says so. Names are given when an area is added, so removing one never renumbers the others. Radius is per area (a park no longer shares one radius). A keyword with no area at all still searches the whole state and shows an "All of South Australia" row with no toggle, removed by clearing the word.
  - **Counts:** each row counts records the way the results do (spatial, keyword, published projects only, plus the projects those records belong to), so a single area's number equals the panel total when it is the only area (checked: Belair 26, Flinders Chase 12, together 38). Overlapping areas can sum to more than the total, which is honest for a union.
  - **Adding an area:** the four method tabs are gone from the card. One "Add area" menu (Draw a circle, Draw a polygon, Enter coordinates, Choose national parks, Upload a shapefile) opens a short flow for that method only, which closes when the area is added (Choose national parks stays open until Done, since it takes several). The search field is now the real `Input` with just the keyword, no chips (the areas list is beside it, so chips would state the same fact twice). Empty state says what to do, once, in the card.
  - **Shared changes, additive:** `SAMap` gained `highlightedBoundaryIds` and `fitRequest` (zoom to one area, same padding rules as the main fit). The coordinate and park radius fields (`InputNumber` with `minValue` 1 and `step` 5) snapped 15 to 16 and 25 to 26 on blur while the state kept 15 and 25, so the field and the search disagreed; `minValue` is now 5 so the steps line up (also fixes option 1).
  - **Verified live on option 3:** two parks give 2 rows with counts and the right total; draw a circle (panel opens, closes when drawn, gets "Circle 1"); enter coordinates ("Point 1"); hide (total drops to the visible area); rename; change radius (25 to 60 km, row updates); remove; upload a GeoJSON (one row "pts.geojson · 2 points", panel closes); hover thickens the shape (stroke 2 to 4); zoom to area changes the scale from 100 km to 10 km; zero console errors; `tsc`, `eslint`, `check:contracts` clean.
  - **Noted, not built (per designer): "exclude" areas.** A layer that removes its records from the union (drawing a hole in a search) is the most layer-like feature and changes the filtering logic. Backlog: revisit if a stakeholder asks for it.
  - **Decision pending (CONTRACTS 4.4, 0.4):** the designer's plan for stakeholders is option 1 as it is, and option 3 becoming the new option 2. That means deleting options 2, 4 and 5 (and their code paths in `observations-search.tsx`) and moving option 3 to `/pages/observations/option-2`. Not done yet: it deletes explored options, so it waits for an explicit go-ahead. Not committed.
- **Sept 25 2026: Explore is down to two options for stakeholders; option 3 became the new option 2, per designer approval ("option 1 - keep as is, option 3 becomes the new option 2").** CONTRACTS 4.4: when a direction is chosen the others are deleted. Option 1 (`/pages/observations`, search on the map, then a results page) is unchanged. Option 2 (`/pages/observations/option-2`) is the floating card with search areas as layers (`ExploreLayout` `"float"`, formerly `"float-grow"`). **Deleted:** the old option 2 (column search and floating results panel, layout `split`), option 4 (card and bottom sheet), option 5 (keyword bar and areas popover), their routes (`option-3`, `option-4`, `option-5`, now 404) and everything only they used: `map-search/floating-results-panel.tsx`, `SearchWithAreas`, the chip row, the sheet snaps, the Areas popover, the "Search areas" layer toggle and the `split` branches. `ExploreLayout` is `"classic" | "float"`, and `ExploreLayoutSwitcher` lists the two options. `SAMap` keeps `zoomPosition` and `fitPaddingBottomRight` (additive props, now only `fitPaddingBottomRight` is used). Nothing else imported the deleted files. Verified: `tsc`, `eslint --max-warnings=0` and `npm run check:contracts` clean; the real Pages build (`PAGES_BASE_PATH=/Biodata npm run build`) passes; live pass for registered and guest: option 1 and option 2 render, options 3 to 5 are 404, the switcher shows two entries, Add area menu and park picker work on option 2, and the only console errors are the expected 404s from visiting the deleted routes. **Older entries above that describe options 3, 4 and 5, or the `split` layout as "option 2", are historical and left as written.**
