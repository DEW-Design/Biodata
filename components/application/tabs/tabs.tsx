...
const getTabStyles = ({ isFocusVisible, isSelected, isHovered }: AriaTabRenderProps) => ({
    ...
    // one instead keeps hover a distinct, neutral `bg-tertiary`/`text-primary` treatment - the same
    // two tokens the icon rail's own non-active hover uses - so a hovered-but-not-yet-selected tab
    // never reads as already selected.
    "button-brand": cx(
        "outline-focus-ring *:data-icon:text-fg-quaternary",
        isFocusVisible && "outline-2 -outline-offset-2",
        isHovered && !isSelected && "bg-tertiary text-primary *:data-icon:text-primary",
        isSelected && "bg-brand-secondary text-brand-secondary *:data-icon:text-fg-brand-secondary_hover",
    ),
    "button-gray": cx(
        "outline-focus-ring *:data-icon:text-fg-quaternary",
        isHovered && "bg-primary_hover text-secondary *:data-icon:text-fg-quaternary_hover",
        isFocusVisible && "outline-2 -outline-offset-2",
        isSelected && "bg-primary_hover text-secondary *:data-icon:text-fg-quaternary_hover",
    ),
    "button-border": cx(
        "outline-focus-ring *:data-icon:text-fg-quaternary",
        isFocusVisible && "outline-2 -outline-offset-2",
        (isSelected || isHovered) && "bg-primary_alt text-secondary shadow-sm *:data-icon:text-fg-quaternary_hover",
    ),
    "button-minimal": cx(
        "rounded-lg outline-focus-ring *:data-icon:text-fg-quaternary",
        isFocusVisible && "outline-2 -outline-offset-2",
        (isSelected || isHovered) && "bg-primary_alt text-secondary shadow-xs ring-1 ring-primary ring-inset *:data-icon:text-fg-quaternary_hover",
    ),
    underline: cx(
        "rounded-none border-b-2 border-transparent outline-focus-ring *:data-icon:text-fg-quaternary",
        isFocusVisible && "outline-2 -outline-offset-2",
        (isSelected || isHovered) && "border-fg-brand-secondary_alt text-brand-secondary *:data-icon:text-fg-brand-secondary_hover",
    ),
    line: cx(
                ...
                className={(state) =>
                    cx(
                        ...
                        orientation === "horizontal" &&
                            type === "underline" &&
                            "relative before:absolute before:inset-x-0 before:bottom-0 before:h-px before:bg-[var(--ui-border-secondary)]",
                    ...
                    <span className={cx("flex items-center gap-1.5", type !== "line" && "px-0.5")}>
                        ...
                        {badge && (
                            <Badge
                                ...
                                // Badge's own `sm` padding (py-1.5/px-2) is tuned for text labels like
                                // "Active" - fine when they're naturally wider than tall regardless. A
                                // short count badge needs less vertical padding to avoid rendering as a
...
export const Tabs = ({ className, ...props }: ComponentPropsWithRef<typeof AriaTabs>) => {
    return (
        <AriaTabs
            ...
            {...props}
            className={(state) => cx("font-barlow flex w-full flex-col", typeof className === "function" ? className(state) : className)}
        />
    );
};
