...
export const ProgressBarCircle = ({ value, min = 0, max = 100, size, label, valueFormatter }: ProgressBarProps) => {
    ...
    return (
        <div className="font-barlow flex flex-col items-center gap-0.5">
            <div role="progressbar" aria-valuenow={value} aria-valuemin={min} aria-valuemax={max} className="relative flex w-max items-center justify-center">
                <svg className="-rotate-90" width={width} height={height} viewBox={viewBox}>
                    {/* Background circle */}
                    <circle
                        // "stroke-bg-quaternary" doesn't compile - "bg-quaternary" isn't a real
                        // colour token in this repo (only text-quaternary is); referencing the
                        // real --ui-bg-tertiary variable directly, same token Toggle's track uses
                        // for the same "unfilled state" role.
                        className="stroke-[var(--ui-bg-tertiary)]"
                        cx={cx}
                        cy={cy}
                        r={radius}
                        fill="none"
