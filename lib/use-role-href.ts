 ...
 * of these was a plain relative path (e.g. `/pages/project-list`) with no query string -
 ...
 *
 * A path that already carries a query string (`/pages/dsa?status=draft`) gets `&userRole=`, not a
 * second `?`.
 */
export function useRoleHref() {
  const role = useUserRole();
  return (path: string) => `${path}${path.includes("?") ? "&" : "?"}userRole=${role}`;
}
